...
void loadVoxelConfig(rclcpp::Node::SharedPtr node, VoxelMapConfig &voxel_config) {
  ...
  voxel_config.max_voxel_size_ = node->get_parameter("lio.voxel_size").as_double();

  node->declare_parameter("lio.min_eigen_value", 0.01);
  voxel_config.planner_threshold_ = node->get_parameter("lio.min_eigen_value").as_double();

  node->declare_parameter("lio.sigma_num", 3.0);
  voxel_config.sigma_num_ = node->get_parameter("lio.sigma_num").as_double();

  node->declare_parameter("lio.beam_err", 0.02);
  voxel_config.beam_err_ = node->get_parameter("lio.beam_err").as_double();

  node->declare_parameter("lio.dept_err", 0.05);
  voxel_config.dept_err_ = node->get_parameter("lio.dept_err").as_double();

  node->declare_parameter("lio.layer_init_num", std::vector<int64_t>{5, 5, 5, 5, 5});
  std::vector<int64_t> layer_init_temp = node->get_parameter("lio.layer_init_num").as_integer_array();
  voxel_config.layer_init_num_ = std::vector<int>(layer_init_temp.begin(), layer_init_temp.end());

  node->declare_parameter("lio.max_points_num", 50);
  voxel_config.max_points_num_ = node->get_parameter("lio.max_points_num").as_int();

  node->declare_parameter("lio.max_iterations", 5);
  voxel_config.max_iterations_ = node->get_parameter("lio.max_iterations").as_int();

  node->declare_parameter("local_map.map_sliding_en", false);
  voxel_config.map_sliding_en = node->get_parameter("local_map.map_sliding_en").as_bool();

  node->declare_parameter("local_map.half_map_size", 100);
  voxel_config.half_map_size = node->get_parameter("local_map.half_map_size").as_int();

  node->declare_parameter("local_map.sliding_thresh", 8.0);
  voxel_config.sliding_thresh = node->get_parameter("local_map.sliding_thresh").as_double();
}
...
void VoxelMapManager::StateEstimation(StatesGroup &state_propagat) {
  ...
  // 提前计算body_cov_list_和cross_mat_list_
  ...
  for (int iterCount = 0; iterCount < config_setting_.max_iterations_;
    ...
    BuildResidualListLRU(pv_list_, ptpl_list_);
    ...
    // IESKF更新
    // 公式11其1 K = (H^T * R^-1 * H + P)^-1 * H^T * R^-1
    // HTz为 H^T * R^-1 * -z
    ...
    // 公式11其2 x_(k+1) = x_k ⊞ dx
    // dx = -K * z - (I - K * H)(x_k ⊟ x_predict)
    // vec 为 (x_predict ⊟ x_k)
    auto vec = state_propagat - state_;
    // solution为dx
    // dx = -K * z - (x_k ⊟ x_predict) + K * H * (x_k ⊟ x_predict)
...
#pragma omp parallel for
  ...
  for (int i = 0; i < index.size(); i++) {
    ...
    if (iter != voxel_map_.end()) {
      ...
      double prob = 0;
      BuildSingleResidual(pv, current_octo, 0, is_sucess, prob, single_ptpl);
      if (!is_sucess) {
        ...
        if (iter_near != voxel_map_.end()) {
          BuildSingleResidual(pv, iter_near->second, 0, is_sucess, prob,
                              single_ptpl);
        }
      }
      if (is_sucess) {
  ...
  for (int i = 0; i < index.size(); i++) {
    ...
    if (iter != vm_map_.end()) {
      ...
      if (!is_sucess) {
        ...
        auto iter_near = vm_map_.find(near_position);
        if (iter_near != vm_map_.end()) {
          BuildSingleResidual(pv, iter_near->second->second, 0, is_sucess, prob,
                              single_ptpl);
        }
      }
      if (is_sucess) {
...
void VoxelMapManager::BuildSingleResidual(pointWithVar &pv,
    ...
    if (current_layer < max_layer) {
      for (size_t leafnum = 0; leafnum < 8; leafnum++) {
        if (current_octo->leaves_[leafnum] != nullptr) {
          ...
          BuildSingleResidual(pv, leaf_octo, current_layer + 1, is_sucess, prob,
                              single_ptpl);
        }
      }
      return;
...
void VoxelMapManager::PubSinglePlane(visualization_msgs::msg::MarkerArray &plane_pub,
  ...
  plane.header.stamp.sec = 0;
  plane.header.stamp.nanosec = 0;
  plane.ns = plane_ns;
  ...
  plane.color.g = rgb(1);
  plane.color.b = rgb(2);
  plane.lifetime.sec = 0;
  plane.lifetime.nanosec = 0;
