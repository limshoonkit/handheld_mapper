...
int width, height;
std::shared_ptr<image_transport::Publisher> pub;
...
static void *WorkThread(void *pUser) {
  ...
  while (!exit_flag && rclcpp::ok()) {
    ...
    if (nRet == MV_OK) {
      ...
      rclcpp::Time rcv_time = node->get_clock()->now();
...
int main(int argc, char **argv) {
  ...
  node = std::make_shared<rclcpp::Node>("mvs_trigger");
  
  if (argc < 2) {
    RCLCPP_ERROR(node->get_logger(), "Usage: mvs_trigger <params_file>");
    rclcpp::shutdown();
    return -1;
  }
  ...
  pub = std::make_shared<image_transport::Publisher>(it.advertise(pub_topic, 1));
