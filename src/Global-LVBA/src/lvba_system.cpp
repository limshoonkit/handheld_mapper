...
LvbaSystem::LvbaSystem(rclcpp::Node::SharedPtr node) : node_(node)
{
    dataset_io_.reset(new DatasetIO(node_));

    auto qos_latched = rclcpp::QoS(1).transient_local();
    cloud_pub_after_  = node_->create_publisher<sensor_msgs::msg::PointCloud2>("/lvba/cloud_after", qos_latched);
    cloud_pub_before_ = node_->create_publisher<sensor_msgs::msg::PointCloud2>("/lvba/cloud_before", qos_latched);
    pub_test_ = node_->create_publisher<sensor_msgs::msg::PointCloud2>("/map_test", 100);
    pub_path_ = node_->create_publisher<sensor_msgs::msg::PointCloud2>("/map_path", 100);
    pub_show_ = node_->create_publisher<sensor_msgs::msg::PointCloud2>("/map_show", 100);
    pub_cute_ = node_->create_publisher<sensor_msgs::msg::PointCloud2>("/map_cute", 100);

    pub_cloud_before_ = node_->create_publisher<sensor_msgs::msg::PointCloud2>("viz/cloud_before", qos_latched);
    pub_cloud_after_  = node_->create_publisher<sensor_msgs::msg::PointCloud2>("viz/cloud_after", qos_latched);
...
template <typename T>
void LvbaSystem::pub_pl_func(T &pl, PointCloud2Pub &pub)
{
