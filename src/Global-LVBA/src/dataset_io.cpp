...
DatasetIO::DatasetIO(rclcpp::Node::SharedPtr node)
{
    readParameters(node);
...
void DatasetIO::readParameters(rclcpp::Node::SharedPtr node)
{
    ...
    // ROS2 only supports vector<double> params; convert to float after reading
    auto s1_ratio_d = node->declare_parameter<std::vector<double>>(
        "BALM_stage1.eigen_ratio_array", std::vector<double>{0.3, 0.1, 0.06, 0.03});
    stage1_eigen_ratio_array_.assign(s1_ratio_d.begin(), s1_ratio_d.end());

    auto s2_ratio_d = node->declare_parameter<std::vector<double>>(
        "BALM_stage2.eigen_ratio_array", std::vector<double>{0.08, 0.08, 0.08, 0.08});
    stage2_eigen_ratio_array_.assign(s2_ratio_d.begin(), s2_ratio_d.end());
