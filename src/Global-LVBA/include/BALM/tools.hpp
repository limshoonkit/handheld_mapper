...
void down_sampling_voxel(pcl::PointCloud<PointT> &pl_feat, double voxel_size)
{
  if (voxel_size < 0.001) return;
  struct AccumVoxel {
    PointT pt;
  ...
  std::unordered_map<VOXEL_LOC, AccumVoxel> feat_map;
  feat_map.reserve(pl_feat.size());

  for (const PointT &p_c : pl_feat.points)
  {
    float loc_xyz[3];
    loc_xyz[0] = static_cast<float>(p_c.x / voxel_size);
    loc_xyz[1] = static_cast<float>(p_c.y / voxel_size);
    loc_xyz[2] = static_cast<float>(p_c.z / voxel_size);

    if (loc_xyz[0] < 0.0f) loc_xyz[0] -= 1.0f;
    if (loc_xyz[1] < 0.0f) loc_xyz[1] -= 1.0f;
    if (loc_xyz[2] < 0.0f) loc_xyz[2] -= 1.0f;

    VOXEL_LOC position((int64_t)loc_xyz[0], (int64_t)loc_xyz[1], (int64_t)loc_xyz[2]);
    auto &acc = feat_map[position];
    if (acc.count == 0) {
      acc.pt = p_c;
    }
    acc.sx += static_cast<double>(p_c.x);
    acc.sy += static_cast<double>(p_c.y);
    acc.sz += static_cast<double>(p_c.z);
    acc.count += 1;
  }

  pcl::PointCloud<PointT> pl_feat2;
  pl_feat.swap(pl_feat2);
  pl_feat.reserve(feat_map.size());
  for (auto iter = feat_map.begin(); iter != feat_map.end(); ++iter) {
    auto &acc = iter->second;
    if (acc.count == 0) continue;
    acc.pt.x = static_cast<float>(acc.sx / acc.count);
    acc.pt.y = static_cast<float>(acc.sy / acc.count);
    acc.pt.z = static_cast<float>(acc.sz / acc.count);
    pl_feat.push_back(acc.pt);
  }
}
...
void down_sampling_voxel2(pcl::PointCloud<PointT> &pl_feat, double voxel_size)
{
  ...
  for (const PointT &p : pl_feat.points)
  {
    ...
    float loc_z = static_cast<float>(p.z / voxel_size);

    if (loc_x < 0.f) loc_x -= 1.f;
    if (loc_y < 0.f) loc_y -= 1.f;
    if (loc_z < 0.f) loc_z -= 1.f;

    const int64_t ix = static_cast<int64_t>(loc_x); 
    const int64_t iy = static_cast<int64_t>(loc_y);
    const int64_t iz = static_cast<int64_t>(loc_z);

    VOXEL_LOC key(ix, iy, iz);

    // voxel geometric center
    const double cx = (static_cast<double>(ix) + 0.5) * voxel_size;
    const double cy = (static_cast<double>(iy) + 0.5) * voxel_size;
    const double cz = (static_cast<double>(iz) + 0.5) * voxel_size;
    ...
    auto &best = feat_map[key];
    if (!best.inited || d2 < best.best_d2) {
      best.pt = p;       // keep this original point
      best.best_d2 = d2;
      best.inited = true;
    }
  }
  ...
  for (const auto &kv : feat_map) {
    pl_feat.push_back(kv.second.pt);
  }
  pl_feat.width = static_cast<uint32_t>(pl_feat.size());
  pl_feat.height = 1;
...
inline constexpr double threshold = 0.1;
