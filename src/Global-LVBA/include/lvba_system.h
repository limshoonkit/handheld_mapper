...
class LvbaSystem {
public:
    LvbaSystem(rclcpp::Node::SharedPtr node);
    ~LvbaSystem() = default;
    ...
    PointCloud2Pub pub_path_, pub_test_, pub_show_, pub_cute_, pub_cloud_before_, pub_cloud_after_, pub_cloud_map_;
    void data_show(vector<IMUST> x_buf, vector<pcl::PointCloud<PointType>::Ptr> &pl_fulls);
    template <typename T> void pub_pl_func(T &pl, PointCloud2Pub &pub);
    ...
    PointCloud2Pub cloud_pub_after_;
    PointCloud2Pub cloud_pub_before_;
